...
from utils_LS.dataset_management import LS_dataset_VolperSubj, LS_dataset_VolperSubjSave
...
def train_loop(epoch, train_loader, model, optimizer, loss_fn, device, num_augment = 0, num_vol=None):
    ''' Training loop used during model selection in RayTune. This function use PyTorch utils to calculate the loss and perform backpropagation.
    ...
    for inputs, labels in train_loader:
        ...
        subj_set_train = LS_dataset_VolperSubj(inputs, labels, num_vol=num_vol, num_augment=num_augment)
        subj_loader_train = DataLoader(subj_set_train, batch_size=1, shuffle=True)
...
def val_loop(epoch, val_loader, model, loss_fn, device, num_augment = 0, num_vol=None, lr_decay = None):
    ''' Validation loop used during model selection in RayTune. This function calculate th loss in the validation set.
    ...
    with t.no_grad():
        ...
        for val_inputs, val_labels in val_loader:
            ...
            subj_set_val = LS_dataset_VolperSubj(val_inputs, val_labels, num_vol=num_vol, num_augment=num_augment)
            subj_loader_val = DataLoader(subj_set_val, batch_size=1, shuffle=False)
...
def train_and_eval(epochs, train_loader, val_loader, model, optimizer, loss_fn, device, num_augment = 0, model_savename = None, num_vol = None):
    ...
    for epoch in range(1, epochs + 1):
        ...
        loss_train = train_loop_tqdm(epoch, train_loader, model, optimizer, loss_fn, device, num_augment=num_augment, num_vol=num_vol)

        loss_val, hist_val = val_loop_tqdm(epoch, val_loader, model, loss_fn, device, num_augment=num_augment, num_vol=num_vol)
...
def take_a_look(model, loader, device, subindex = 0, figsize=(10,10)):
    ...
    with t.no_grad():
        ...
        for inputs, labels in loader:
                if i == subindex:
                        subj_set_test = LS_dataset_VolperSubj(inputs, labels)
                        subj_loader_test = DataLoader(subj_set_test, batch_size=1, shuffle=False)
    ...
    '''
     with t.no_grad():
        ...
        for subject, SubjectID in test_loader:
                ...
                subj_set_test = LS_dataset_VolperSubjSave(subject)
                subj_loader_test = DataLoader(subj_set_test, batch_size=1, shuffle=False)
                volnr = 0
